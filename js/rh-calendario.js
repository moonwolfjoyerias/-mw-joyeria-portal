...
// mismo modelo compartido (js/eventos-modelo.js — clave de
...
function renderTablaCalendario() {
  ...
  tbody.querySelectorAll('[data-asistencia]').forEach(btn => {
    btn.addEventListener('click', () => abrirModalAsistenciaEvento(eventosCalendario.find(ev => ev.id === btn.dataset.asistencia)));
  });
